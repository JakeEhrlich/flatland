...
Default output: `build/assembly/<name>-bom.csv`.

## pcb pnp

One row per placed, assembled part.
...
* `Mid X/Y` are the footprint origin in millimetres in the board frame —
  the same origin and y-up orientation as the gerbers, which is what the
  assembler aligns to.
